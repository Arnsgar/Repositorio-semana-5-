    // Función para alternar la apertura y cierre de los grupos de investigación
    function abrirGrupo(element) {
        // Primero cerrar todos los demás grupos
        const grupos = document.querySelectorAll('.pestaña');
        grupos.forEach(grupo => {
            grupo.classList.remove('activo');
            grupo.querySelector('.contenidoGI').classList.remove('mostrar'); // Cierra todo el contenido de los grupos
        });

        // Abrir el grupo seleccionado
        element.classList.add('activo');
        element.querySelector('.contenidoGI').classList.add('mostrar'); // Muestra el contenido del grupo seleccionado
    }
    ...
    // Función para alternar la apertura y cierre de los grupos de investigación
function abrirGrupo(element) {
    // Primero, verifica si el grupo ya está activo
    const contenido = element.querySelector('.contenidoGI');

    if (element.classList.contains('activo')) {
        // Si está activo, ciérralo
        element.classList.remove('activo');
        contenido.classList.remove('mostrar'); // Oculta el contenido del grupo
    } else {
        // Si no está activo, cierra todos los demás grupos
        const grupos = document.querySelectorAll('.pestaña');
        grupos.forEach(grupo => {
            grupo.classList.remove('activo');
            grupo.querySelector('.contenidoGI').classList.remove('mostrar'); // Cierra todo el contenido de los grupos
        ...
        element.classList.add('activo');
        contenido.classList.add('mostrar'); // Muestra el contenido del grupo seleccionado
    }
}
